...
int main(int argc, char** argv) {
    ...
    const std::filesystem::path input{"/tmp/musicrat-player-input.aac"};
    const std::filesystem::path output{"/tmp/musicrat-aac-player-output.wav"};
    std::filesystem::remove(input);
    std::filesystem::remove(output);

    if (!write_aac_fixture(input)) {
        return 2;
    }
