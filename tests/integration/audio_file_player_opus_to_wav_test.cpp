...
int main(int argc, char** argv) {
    ...
    const std::filesystem::path input{"/tmp/musicrat-player-input.opus"};
    const std::filesystem::path output{"/tmp/musicrat-opus-player-output.wav"};
    std::filesystem::remove(input);
    std::filesystem::remove(output);

    if (!write_opus_fixture(input)) {
        return 2;
    }
