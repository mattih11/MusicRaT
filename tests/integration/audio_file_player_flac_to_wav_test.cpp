...
int main(int argc, char** argv) {
    ...
    const std::filesystem::path input{"/tmp/musicrat-player-input.flac"};
    const std::filesystem::path output{"/tmp/musicrat-flac-player-output.wav"};
    std::filesystem::remove(input);
    std::filesystem::remove(output);

    std::vector<FLAC__int32> samples(80000);
    for (std::size_t frame = 0; frame < samples.size(); ++frame) {
        samples[frame] = (frame % 64) < 32 ? 8192 : -8192;
    }
    if (!write_flac_fixture(input, 48000, 1, samples)) {
        return 2;
    }
